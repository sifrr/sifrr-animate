...
describe('Bezier', function() {
  it('has correct values', async () => {
    const bezier = Bezier.fromArray([0.5, 0.5, 0.5, 0.5]);

    expect(bezier(0.1)).to.equal(0.1);
    expect(bezier(0.6)).to.equal(0.6);
    expect(bezier(1)).to.equal(1);

    const bezier1 = Bezier.fromArray([0.17, 0.67, 0.83, 0.67]);
    ...
    const bezier2 = Bezier.fromArray([0, 1, 0, 1]);
    expect(Math.round(bezier2(0) * 100) / 100).to.equal(0);
  });

  it('returns memoized values', async () => {
    const bezier1 = Bezier.fromArray([0.1, 0.9, 0.3, 0.7]);
    const bezier2 = Bezier.fromArray([0.1, 0.9, 0.3, 0.7]);
    const bezier3 = Bezier.fromArray([0.1, 0.9, 0.3, 0.7]);
